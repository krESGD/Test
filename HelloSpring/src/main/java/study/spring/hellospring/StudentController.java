...
public class StudentController {
	...
	// 목록, 상세보기에서 사용할 서비스 객체 --> Department와 Join처리
	// --> import study.spring.hellospring.service.StudentJoinService;
	@Autowired
	...
	// 등록, 수정시에 소속교수에 대한 드롭다운을 구현하기 위한 서비스 객체
	// --> import study.spring.hellospring.service.ProfessorService;
	@Autowired
	...
	public ModelAndView studList(Locale locale, Model model) {
		...
		int totalCount = 0;
		try {
			// StudentMapper.selectStudentList 기능을 호출한다.
		...
		// 페이지 번호에 대한 연산 수행 후 조회조건 값 지정을 위한 Beans에 추가하기
		...
		try {
			// ProfessorMapper.selectDepartmentList 기능을 호출한다.
			// 두 번째 파라미터는 조회 조건시에 사용될 파라미터 --> Beans객체
			// 조회 결과가 다중행을 리턴하기 때문에 List형태로 리턴된다.
	...
	public ModelAndView studView(Locale locale, Model model) {
		...
		try {
			item = studentJoinService.get_StudentJoin_item(student);
		} catch(Exception e) {
			return web.redirect(null, e.getLocalizedMessage());
		}
	...
	public ModelAndView studAdd(Locale locale, Model model) {
		...
		try {
			// DepartmentMapper.selectDepartmentList 기능을 호출한다.
			// 두 번째 파라미터는 조회 조건시에 사용될 파라미터 --> Beans객체
			// 조회 결과가 다중행을 리턴하기 때문에 List형태로 리턴된다.
	...
	public ModelAndView studAddOk(Locale locale, Model model) {
		...
		if (idnum == null) {
			return web.redirect(null, "주민번호를 입력하세요.");
		}
		...
		if (deptno == 0) {
			return web.redirect(null, "소속학과를 입력하세요.");
		}
